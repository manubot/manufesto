Scholarly publishing is outdated.
The process by which articles are written, reviewed, and published affects the pace of scholarly progress.
Unfortunately, the current process predisposes research to be irreproducible, uncollaborative, proprietary, poorly documented, and difficult to build upon.
...
But how?
Here we envision the functionality and principles that would define an ideal platform.
...
Manubot implements many of the features described below; the rest are goals.
...
In the meantime, we must experiment, explore the pitfalls of innovations, and establish best practices, such that when scholars are ready to migrate en masse, the path forward is clear.
...
Currently, article processing charges (formerly subscription revenue) do not fund the intellectual contributions or creation of manuscripts.
Instead they fund inefficient publishing platforms that can either be fully replaced by automation or the existing volunteer contributions of authors and reviewers.
There will always be some cost for compute, hosting, and archiving of scholarly communication, but these should be in the range of dollars per article not thousands and are currently offered for free by several services.
...
#. **Journals should have public issue trackers** and accept community pull requests to improve their infrastructure. Journals are buggy and publishers currently make many mistakes.
Even popular preprint servers do not have issue trackers that allow users to discuss bugs or suggest improvements.
Currently, when there’s a problem with a journal, the public can either email the journal or tweet.
Neither method is well-suited to continuous public discussion and oversight that is critical to transparency and making sure the best solutions are pursued.
Did the publisher submit incorrect metadata to Crossref or PubMed?
Submit a pull request the fix the metadata depositing software and fix not only your paper’s metadata but also all other records with the same issue.
...
Currently, journals sometimes ask authors to submit a tracked changes document, while preprint servers don’t provide any diffs at all.
Ideally, diffs would automatically be generated from two manuscript versions without extra effort by the author.
...
No more errata or corrigenda as separate publications: just a new version with a note of who changed what and why.
...
Online writing in a public venue reduces the hurdles to contribute to existing projects, tipping the scales to encourage global collaboration rather than fragmentation.
For some studies, community members may only contribute small enhancements, like fixing typos.
Other studies, may become massively open online papers (MOOPs) where geographically dispersed experts co-investigate at scale.

TODO: fix redundancy:
Community members should be able to propose contributions that are reviewed by manuscript maintainers.
...
The formatting changes and copyedits performed by journal should receive the same transparency and scrutiny as those performed by the authors themselves.
...
Post publication peer review enables anyone interested in the study to submit feedback.
Many times, a post publication reviewer will have scrutinized a study much more closely.
Furthermore, we should collect as much discussion as possible on the merits of a study.
...
The reference to the upstream analysis should use content-addressing to specify the exact source code and data that generated the value.
In practice, this means readers can go backwards from a value or figure to the source code that generated it.
...
when and by whom was it originally written? What subsequent edits have been made and why?
...
Attribution helps keep authors honest.
...
When reading a manuscript, it should be easy to retrieve all relevant conversation, experimentation, and documentation relating to a specific section or finding.
...
As this begins to occur, we need a publication system that ensures these artifacts are easily retrievable from the final product.

#. **Academic misconduct should be infeasible.**
Misconduct thrives when perpetrators thinks there’s a strong possibility it goes undetected.
...
One commonality is that public scrutiny is enough sufficient to dissuade such behavior.
Therefore, transparency in research and publishing is essential.

Furthermore, we should look to technical solutions to tip practitioners towards honesty and ethical choices rather than misconduct.
One example is timestamping manuscripts such that authors can prove that a given version existed at a given time.
...
Alternatively, credit is silently deprived from the true creators of content, tending to favor established scientists to the detriment of early career and disadvantaged scientists.
But a transparent scholarly record means authorship can be precisely determined and displayed alongside research.
...
The best example is how journals email authors a PDF proof to review due back in 24 hours.
The PDF contains unknown changes from the submitted document.
...
A variety of mistakes are uncovering ranging from introduced typos, editor mistakes, formatting issues, and other conversion issues.
Authors must annotate the PDF with suggested changes.
The journal ignores most of the author suggestions and publishes the error-ridden manuscript.

None of this needs to be.
By automating publishing with tested computational workflows, manuscripts can be published with perfect fidelity.
...
For example, if an author’s affiliation changes, there should be a single source field that is modified.
...
In contrast, PIDs often include sufficient metadata to fully populate the bibliography and references can be automatically generated for thousands of existing bibliographic styles.
...
For example, the modifications could be automatically relayed to the publisher.
...
However, not all citations are equal: some dispute the cited work, some affirm it, most do neither.
...
One contributing factor is that interfaces have not been portable between journals, due to their proprietary and non-modular implementation.
...
For example, it should be possible to go from a statement to excerpts from other studies commenting on the same topic.
...
With the explosion in the amount of literature produced, machines are crucial to help scholars find the right literature as well as build knowledge graphs that aid human investigation by scaling storage beyond the capacity of the brain.
Machine-readable manuscripts need to conform to standards and expose as much metadata as possible.
...
Therefore, access to articles won’t shut off due to downtime or technical difficulties at the publisher’s site.
